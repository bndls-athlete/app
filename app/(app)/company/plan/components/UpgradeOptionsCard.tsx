...
import { getBrandSubscriptionButtonData } from "@/helpers/stripeBrandManager";
...
const UpgradeOptionsCard = ({
  ...
  const handleSwitch = async () => {
    setIsLoading(true);
    try {
      const { data } = await axios.post("/api/stripe/company/net-difference", {
        newPriceId: priceId,
      });
  ...
  const executeSwitch = async () => {
    ...
    try {
      await axios.post("/api/stripe/company/switch-plan", {
        newPriceId: priceId,
      });
  ...
  const handleManageBilling = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get("/api/stripe/company/manage-billing");
  ...
  const proceedWithCheckout = async (priceId: string) => {
    ...
    try {
      ...
      const { data: sessionData } = await axios.post(
        "/api/stripe/company/checkout-session",
        {
          priceId,
        }
      );
  ...
  const { text: buttonText, action: buttonAction } =
    getBrandSubscriptionButtonData({
      brand: brand!,
      planPriceId: priceId,
      handleSwitch,
      handleManageBilling,
